...
//
// 为什么以 layout.json 为主入口而不是 content_list.json：
//   - layout.json 提供 span 级 bbox；content_list.json 只有段落级 bbox。
...
// 把 layout.json 的字节内容适配成 ExtractedDocument。
...
// 目录需包含 layout.json；images/ 存在时按需读取（当前仅记录，不预载）。
...
// ZIP 条目会经过路径校验（拒绝绝对路径、`..` 穿越、符号链接）与体积上限检查，
