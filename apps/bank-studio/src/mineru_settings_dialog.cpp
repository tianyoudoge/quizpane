...
std::optional<MineruConfig> editMineruSettings(QWidget* parent, const MineruConfig& current,
    ...
    dialog.setObjectName(QStringLiteral("mineruSettingsDialog"));
    ...
    auto* links = new QLabel(QStringLiteral(
        "PDF 和图片会上传至 MinerU 解析　"
        "<a href=\"https://mineru.net/apiManage/token\">获取 Token</a> · "
        "<a href=\"https://mineru.net/apiManage/docs\">帮助</a>"));
    links->setObjectName(QStringLiteral("muted"));
    ...
    quotaPanel->setObjectName(QStringLiteral("mineruQuotaSummary"));
    ...
    quotaPanel->setText(QStringLiteral("今日已提交 %1 个文件").arg(localSubmittedFiles()));
    auto* refreshQuota = new QPushButton(QStringLiteral("查看额度"));
    refreshQuota->setObjectName(QStringLiteral("secondaryButton"));
    auto* quotaRow = new QHBoxLayout;
    quotaRow->addWidget(quotaPanel, 1);
    quotaRow->addWidget(refreshQuota);
    ...
    QObject::connect(refreshQuota, &QPushButton::clicked, &dialog,
        ...
        if (value.isEmpty()) {
            quotaPanel->setText(QStringLiteral("填写 Token 后可查看额度"));
            return;
        }
        refreshQuota->setEnabled(false);
        refreshQuota->setText(QStringLiteral("查询中…"));
        ...
        QObject::connect(reply, &QNetworkReply::finished, quotaManager,
            ...
            if (status < 400 && json.object().value(QStringLiteral("code")).toInt(-1) == 0 &&
                ...
                quotaPanel->setText(QStringLiteral("今日已提交 %1 个文件　·　高优页数剩余 %2 页")
                    .arg(localSubmittedFiles())
                    .arg(data.value(QStringLiteral("user_left_quota")).toInt()));
                refreshQuota->setText(QStringLiteral("刷新"));
                return;
            }
            // 不显示 HTTP、服务端 msg 或网络库错误，避免暴露服务实现细节。
            quotaPanel->setText(QStringLiteral("今日已提交 %1 个文件　·　额度暂时无法查询")
                                    .arg(localSubmittedFiles()));
            refreshQuota->setText(QStringLiteral("重试"));
        });
    });
