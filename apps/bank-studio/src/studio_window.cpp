...
QWidget* StudioWindow::buildFinishPage() {
    ...
    count->addItems({QStringLiteral("5 题"), QStringLiteral("10 题"),
...
void StudioWindow::beginPreflight() {
    ...
    // 当前 MVP 只做确定性的本地预检；模型调用尚未接入，因此 Token 是按文件
    // 字节数估算的输入上限，不代表真实计费数据。
    preflightStep_ = 0;
    estimatedInputTokens_ = 0;
...
void StudioWindow::runPreflightStep() {
    ...
    phaseDetail_->setText(QStringLiteral(
        "模型调用尚未接入，因此没有发送资料，也没有产生输出 Token。"
        "后续可从此检查点继续。"));
