...
MVP 支持单选、多选和判断题。填空、简答和材料题应在后续协议版本加入，不能让 Agent 用非标准字段偷偷表达。
...
4. 外部 Agent：提供 `SKILL.md`、JSON Schema 和无编译打包脚本，支持 Codex、Claude Code 等直接生成包。
...
2. **选择处理方式**：推荐模型、本地模型或填写自己的 API；
3. **自动整理**：显示“已发现 126 题，12 题需要确认”，允许后台继续；
4. **检查问题**：只展示缺答案、选项错位、疑似重复等异常题，正常题默认折叠；
5. **完成**：填写题库名称，点击“生成并安装”。
...
7. 增加外部 Agent Skill、无开发环境使用说明和回归语料集。
