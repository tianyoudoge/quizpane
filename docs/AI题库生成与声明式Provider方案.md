...
MVP 支持单选和判断题。多选、填空和简答应在后续协议版本加入，不能让 Agent 用非标准字段偷偷表达。共享材料与题组采用独立的 Schema v2 原生建模，实施与验收要求见 [共享材料与题组支持交接方案](共享材料与题组支持交接方案.md)；明确禁止把材料复制到每道题的 `stem` 作为兼容兜底。多选需要主程序同步改为多选控件后再提升对应 Schema 能力。
...
第一阶段按段落和 token 上限顺序切块，并给每块稳定编号。模型每次只输出包含 `questions` 数组的 JSON 对象，不直接生成 ZIP，也不负责文件路径、Manifest 或版本号。这样可以断点续跑，并限制一次错误的影响范围；按页/标题切分与并发处理留待对应提取器接入后实现。
