...
规范文件见 `schemas/declarative-provider.schema.json`。核心概念如下：
...
MVP 支持单选和判断题。当前只存在 `schemaVersion: 2`，不接受或迁移 v1。共享材料与题组通过顶层 `materials` 和题目 `materialId` 原生表达；明确禁止把材料复制到每道题的 `stem` 作为兼容兜底。多选、填空和简答必须在生产端具备完整能力后再修改唯一 Schema，不能用非标准字段偷偷表达。
...
切块先识别材料标题、题组提示、连续编号选项题和 Markdown 标题，形成稳定 `SourceBlock`；材料与其子题属于不可拆分单元。多个普通 `SourceBlock` 再按 token 上限装配为一次 `TextChunk` 请求。模型每次只输出包含 `materials` 与 `questions` 数组的 JSON 对象，不直接生成 ZIP，也不负责文件路径、Manifest 或版本号。检查点以 `SourceBlock` 而不是字符切片为完成单位，因此调整请求预算不会破坏恢复语义。

模型适配层全部内置在“题库制作器”中，用户不需要安装任何 Agent、命令行或开发环境：

1. MVP：OpenAI-compatible Chat Completions 接口，兼容用户自备 API 和多数网关；
...
题库制作器自身就是 Agent Host。用户只看到图形界面，内部模块关系如下：
...
不得把 Codex、Claude Code、Python、Node.js、Docker 或任何 Agent CLI 作为最终用户依赖。开发阶段可以使用这些工具生成测试数据，但发行版功能必须在只有小窗刷题和题库制作器的干净系统上完整运行。
...
- 材料 ID 必须唯一，题目 `materialId` 必须引用真实材料且分类一致；
- 每份材料至少被一道题引用，材料正文不得完整复制进关联题目的 `stem`。
...
校验器把结构化错误、相关原文块和上一轮完整 `materials + questions` 候选一起发回模型，修复材料及其全部关联题目，禁止只回传单题破坏聚合上下文。仍失败的题进入人工复核列表，禁止无限重试。打包器只接受校验通过的 canonical JSON，因此更换模型不会改变最终质量门槛。

### 6.4 可复现性

- 保存源文件 SHA-256、稳定源块进度、材料重命名映射、材料关联题目和 Token 累计值；
- 材料和题目 ID 由源块序号、块内序号及规范化模型 ID 组合生成，不直接信任模型 ID；
