...
> 代码位置最近于 2026-08-30 对照提交 `9415c61` 核验；Release 配置增量构建成功，
> `ctest --preset release` 28/28 通过。行号可能随代码演进漂移，函数/类名比行号更稳定。

> 2026-09-05 Win7 x86 修复增量：Qt6 Release（含 PDF/OCR）及 macOS Qt5.15.19
> 兼容配置（无 PDF）均构建成功、各 30/30 测试通过。2026-09-20 Win7 x64 已完成
> 147 页/600 题本地规则链路及真实 MinerU 失败恢复验收；用户明确 x86 不属于发布验收范围。

## 维护约定

- 开发前先通过本目录定位数据流、设计约束、代码入口与回归测试；再以当前代码和测试确认实际行为。
- 代码与 spec 冲突时，不盲信旧行号；先用稳定符号和调用链核对，再判断是实现回归还是文档漂移。
- 改动用户可见行为、数据契约、识别规则、失败语义或已知限制时，必须在同一次变更中同步相关 spec；已解决的 `todo.md` 项不得继续写成未实现。
...
| [06-Win7-x86兼容性与低风险性能优化审阅.md](06-Win7-x86兼容性与低风险性能优化审阅.md) | Win7 x86 兼容问题、修复状态、性能优化和实机验收边界 |
| [refactor/](refactor/README.md) | 架构视角导读：分层/组件架构图、五大子系统数据流图（SVG，简单+复杂版）、分模块重构/性能/可读性优化建议。面向服务端背景新人上手 |
