...
- 刷新签名地址期间保留下载重试计数；成功轮询不能把下载失败次数清零。连续下载失败最多
  重试 4 次，随后明确结束并允许下次打开恢复任务，禁止无限停在“1/4”；
- Win7 x64 真实 147 页云任务已用修复包 `49866ea` 续接复验：失败计数到“4/4”后按上限
  结束，弹窗提示任务已保留且可重新打开继续等待；
- 空包失败；结果上限 512 MiB。`readyRead` 每次最多读 64 KiB，网络读缓冲限制为
  256 KiB；已声明超限的 Content-Length 立即中止，无长度响应按实际累计字节检查。
- 使用 `QSaveFile` 写临时文件，完整成功才 `commit()`。取消、超限、网络/写盘失败
  不发布半包，并保留同路径已有完整结果；下载失败仍回到 `poll()` 获取新签名 URL。
  `mineru_client_test` 覆盖分块无长度响应、字节一致性、取消、超限、空包、不可写目标及重试。
...
- **重试**：`isTransientMineruFailure()`（`:70-79`）仅 408/425/429/≥500 与连接层故障（httpStatus==0）算瞬时；
  **`OperationCanceledError`（=5）绝不重试**；已有 4xx 响应视为确定性错误直接给用户。
  ...
  下载失败为刷新签名地址而成功轮询时，仍沿用同一下载重试预算；只有下载成功或开始新任务才清零。
...
4. **规则工作流** `startRuleBased`（`generation_workflow.cpp:25-147`）：
   ...
   进度映射：云解析 0-20、Extracting 20-60、Chunking 60-90、Done 100。
   Chunking 不再停在固定百分比：`RuleGenerationProgress` 在每道题完成后上报当前文件、
   当前分套、套内题号进度和累计已整理/待复核题数，`GenerationWorkflow` 将其映射为真实百分比；
   UI 只呈现阶段、题目进度和简短说明。整页 PNG 缓存、逐题校对图数量/字节及 Windows
   Private Usage 仅每 10 题写入 `rule-progress` 诊断事件，不作为用户进度信息展示。
   日志同时记录正式 `generatedAssets` 和复核 `reviewAssets` 的数量/字节，避免只盯
   页面缓存而漏掉其他图片容器。
   整理运行期间底部不显示“后台等待并关闭”：该旧入口本质是关闭当前进程，不能让本地
   规则线程继续后台运行。用户仍可使用明确的“取消整理/放弃此任务”；直接关闭窗口时沿用
   现有确认框和云端 batchId 恢复语义。
...
ZIP 512MB / layout.json 64MB / 10000 页；下载已流式落盘，但适配器打开 ZIP 时仍整包驻留内存；
