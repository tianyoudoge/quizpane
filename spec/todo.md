...
### A17. Qt 5.15/Win7 按分套重复打开 PDF 导致 PDFium 提交内存累积 — 已通过 x64 实机验收（2026-09-20）
...
- **Win7 x64 实机复验**：修复包 `49866ea` 本地规则解析同一 147 页题本，成功生成 600 题，
  0 个必须处理项，并完成打包、安装及主程序打开。完成阶段 `tasklist` 工作集约
  2,773,924 KiB，未再出现旧版逐套增长到 15.1GB、提交额度耗尽或 OOM；用户已明确
  Win7 x86 不属于本轮发布验收范围。
...
- 发布验收以 Win7 SP1 x64 为准；用户已明确 x86 不属于本轮范围。仍待验收：真实更新/
  PowerShell 2.0 与 5.1、Chrome 109 课程控制。x86 内存阈值仍无实机基准，只作为历史审阅结论，
  不作为 x64 发布阻塞项。
...
### F2. 页面控制器拆分后做题页布局冲突 — 已修复（2026-09-12）

启用 WebSockets 时主窗口已为网课状态栏创建布局，做题控制器又安装新布局，
导致题干、选项及交卷控件未挂入页面。`PracticePageController::buildInto`
现复用已有 `QVBoxLayout`；`practice_page_controller_test` 覆盖有/无预置状态栏，
并检查真实题目与作答控件可见性。续验发现拖拽柄晚于控制器初始化，
进入练习解引用空指针；现已调整构造顺序、加入空值保护及对应回归。
本地 WebSocket 握手/心跳实测通过，扩展协议测试 29/29，Qt6/Qt5 各 34/34 CTest 通过。
平台实机验收范围仍见 F。

### F3. 解析页原生单选判分显示错误 — 已修复（2026-09-12）

仅返回 `correctChoice` 的原生题库缺少 `correctChoices` 数组。答案集合合并后，
未作答空集与缺失答案数组相等，错误显示为“正确”；原有“正确单选仍显示错误”也源于
同一字段误用。现按题型比较，并要求非空作答。回归：`solution_page_controller_test`。

### F4. 复核控制器拆分后进度统计不更新 — 已修复（2026-09-12）
...
2026-09-20 Win7 x64 实机再次回测同一 147 页题本时发现：结果 CDN 持续失败后，
客户端为刷新签名地址回到轮询，而成功轮询错误地清零下载重试次数，界面因此无限停在
“1/4”。现已让签名地址刷新保留下载重试预算；连续失败在 4 次重试后明确结束，
`mineru_client_test` 覆盖持续 503、重试次数上限和不发布半包。
