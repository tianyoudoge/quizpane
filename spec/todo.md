...
- 已实现：x86 渲染预算、扫描/预览入口保护、复核 PDF 会话与有界懒图缓存、批量建树
  暂停更新、MinerU 流式下载、结果图片按页生成。没有修改识别规则；后续按用户确认统一 Win7 默认启用 OCR。
...
产品预期不一致。`build-windows7.ps1` 现默认启用，Release 显式启用并增加模型存在检查、
解压包 OCR/损坏模型/导入 API 验证；默认产物路径仍为 `dist/windows7/`。
无 OCR 仅作为显式诊断选项保留。此前 Actions 无 OCR 包不能作为本轮 OCR 验收结果。
