...
Nginx，不需要新增 DNS 记录。模板不会处理域名根路径 `/`，该位置会返回 404；
如需个人主页，请在同一个 `server` 的 `location /` 中配置你的个人站点目录或反向代理。

若 Nginx 日志出现 `current/index.html" is forbidden (13: Permission denied)`，是
`www-data` 无法穿透静态站目录。可安全执行：

```bash
sudo chmod 0755 /srv/quizpane /srv/quizpane/site /srv/quizpane/site/releases
sudo find /srv/quizpane/site/releases -type d -exec chmod 0755 {} +
sudo find /srv/quizpane/site/releases -type f -exec chmod 0644 {} +
